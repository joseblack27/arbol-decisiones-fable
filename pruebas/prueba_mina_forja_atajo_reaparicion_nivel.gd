# =============================================================================
# Prueba: NivelMina repone al Núcleo de la Forja en la MISMA ruta de nodo
# tras morir Y abre/cierra su PROPIA compuerta (CompuertaAtajoForja) — mirror
# exacto de prueba_mina_atajo_reaparicion_nivel.gd (ver ese archivo para el
# historial completo), pero para el segundo bloque jefe/compuerta agregado
# en la rama Forja de Fuego. Usa un nivel SINTÉTICO (script + contenedores
# "Enemigos"/"Decoraciones" + jefe + compuerta), no el .tscn real completo.
#
# Simula ser el SERVIDOR con un ENetMultiplayerPeer sin que nadie se
# conecte (mismo criterio que las pruebas hermanas).
#   godot --headless --path . --script res://pruebas/prueba_mina_forja_atajo_reaparicion_nivel.gd
# =============================================================================
extends SceneTree
...
func _crear_nivel_falso() -> Node2D:
	...
	var jefe := (load("res://escenas/enemigos/EnemigoNucleoForja.tscn") as PackedScene).instantiate()
	...
	compuerta.name = "CompuertaAtajoForja"
...
func _montar() -> void:
	...
	peer.create_server(34600)
	root.multiplayer.multiplayer_peer = peer

	_nivel = _crear_nivel_falso()
	root.add_child(_nivel)
	current_scene = _nivel
	_jefe_original = _nivel.get_node(_nivel._RUTA_JEFE_FORJA)
	_jefe_original.global_position = Vector2(741, 852)
	_nivel._posicion_jefe_forja = _jefe_original.global_position
	_compuerta = _nivel.get_node(_nivel._RUTA_COMPUERTA_FORJA)


func _probar_conexion() -> void:
	_conecto_la_muerte_ok = _jefe_original.componente_vida.muerte.is_connected(_nivel._al_morir_jefe_forja)
	print("El nivel escucha la muerte real del jefe de la forja (esperado true): %s" % _conecto_la_muerte_ok)

	_compuerta_cerrada_al_inicio_ok = not _compuerta.abierta
	print("La compuerta de la forja arranca cerrada (esperado true): %s" % _compuerta_cerrada_al_inicio_ok)


func _matar_jefe() -> void:
	_nivel.call("_al_morir_jefe_forja", 0.0)
	_marca_muerto_ok = _nivel._jefe_forja_muerto
	print("_jefe_forja_muerto pasa a true al morir (esperado true): %s" % _marca_muerto_ok)
...
func _verificar_compuerta_abierta() -> void:
	...
	print("La compuerta de la forja se abre al morir el jefe (esperado true): %s" % _compuerta_se_abre_al_morir_ok)


func _pedir_respawn() -> void:
	_nivel.call("_respawnear_jefe_forja")


func _probar_respawn_y_reconexion() -> bool:
	var jefe_nuevo = _nivel.get_node_or_null(_nivel._RUTA_JEFE_FORJA)
	...
	_respawnea_en_la_misma_posicion_ok = jefe_nuevo != null \
		and jefe_nuevo.global_position.is_equal_approx(Vector2(741, 852))
	print("Aparece en la misma posición que el original (esperado true): %s" % \
		_respawnea_en_la_misma_posicion_ok)

	_jefe_muerto_vuelve_a_false_ok = not _nivel._jefe_forja_muerto
	print("_jefe_forja_muerto vuelve a false tras el respawn (esperado true): %s" % _jefe_muerto_vuelve_a_false_ok)

	_jefe_nuevo_muerte_conectada_ok = jefe_nuevo != null \
		and jefe_nuevo.componente_vida.muerte.is_connected(_nivel._al_morir_jefe_forja)
	...
	print("La compuerta de la forja se vuelve a cerrar al reponer al jefe (esperado true): %s" % \
		_compuerta_se_cierra_al_respawnear_ok)

	_nivel.call("_al_peer_listo", 2)
	var jefe_tras_segundo_peer = _nivel.get_node_or_null(_nivel._RUTA_JEFE_FORJA)
...
func _informar() -> bool:
	...
	print("PRUEBA MINA FORJA ATAJO REAPARICION NIVEL %s" % ("OK" if exito else "FALLIDA"))
