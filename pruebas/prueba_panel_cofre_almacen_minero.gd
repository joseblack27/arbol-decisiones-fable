# =============================================================================
# Prueba de PanelCofre en modo almacén del minero — mirror exacto de
# prueba_panel_cofre_almacen_lenador.gd (ver ese archivo para el porqué
# completo de cada decisión). Mismo panel de arrastrar-y-soltar que un
# cofre normal, pero GrillaCofre se puebla con el contenido del almacén
# compartido del minero (ver GestorMinero.gd) a través de
# FuenteAlmacenMinero en vez de FuenteCofre/FuenteAlmacenLenador.
#
# A diferencia del mirror del Leñador, acá el "refresco en caliente" y el
# "tomar todo" reusan el ÚNICO ítem de recurso que existe hasta ahora
# (mineral_cristal_1.tres, ver el plan "Mina de Cristal") en vez de un
# segundo tipo distinto — el punto de la prueba (¿aparece solo, sin cerrar
# y reabrir?) no depende de que sea un ítem diferente, solo de que la
# grilla se refresque sola tras un depósito posterior al retiro.
#   godot --headless --path . --script res://pruebas/prueba_panel_cofre_almacen_minero.gd
# =============================================================================
extends SceneTree

const _RUTA_MINERAL := "res://recursos/items/recursos/mineral_cristal_1.tres"

var _jugador
var _inventario
var _panel
var _gm


var _se_abre_con_titulo_y_contenido_del_almacen_ok := false
var _bloquea_depositar_desde_inventario_ok := false
var _retirar_descuenta_del_almacen_y_da_item_ok := false
var _refresco_en_caliente_al_depositar_ok := false
var _tomar_todo_vacia_el_almacen_ok := false


func _process(_delta: float) -> bool:
	_montar()
	_probar_abrir()
	_probar_bloquea_depositar()
	_probar_retirar()
	_probar_refresco_en_caliente()
	_probar_tomar_todo()
	return _informar()


func _montar() -> void:
	_gm = root.get_node("/root/GestorMinero")
	_gm._almacen.clear()
	_gm.almacen_replicado.clear()

	var mineral := load(_RUTA_MINERAL) as DatosItem
	_gm.depositar_servidor(mineral)  # 1 unidad (ver mineral_cristal_1.tres, quantity=1).

	_jugador = (load("res://escenas/jugador/Jugador.tscn") as PackedScene).instantiate()
	root.add_child(_jugador)
	_inventario = _jugador.get_node("InventarioComponente")

	_panel = (load("res://escenas/ui/panel_cofre/PanelCofre.tscn") as PackedScene).instantiate()
	root.add_child(_panel)


func _casilla_con_nombre(grilla: GrillaObjetos, nombre: String) -> Node:
	for casilla in grilla._contenedor.get_children():
		if casilla.item_data and casilla.item_data.name == nombre:
			return casilla
	return null


func _probar_abrir() -> void:
	root.get_node("/root/BusEventos").almacen_minero_solicitado.emit()

	print("Se abre con el título del almacén (esperado 'Almacén del Minero'): %s" % \
		_panel._grilla_cofre.titulo)
	var titulo_ok: bool = _panel.visible and _panel._grilla_cofre.titulo == "Almacén del Minero"

	var casilla_mineral := _casilla_con_nombre(_panel._grilla_cofre, "Mineral de Cristal")
	var contenido_ok: bool = casilla_mineral != null and casilla_mineral.item_data.quantity == 1
	print("La grilla muestra el contenido real del almacén (esperado true, 1): %s, %s" % [
		casilla_mineral != null, casilla_mineral.item_data.quantity if casilla_mineral else -1
	])

	print("Sin filtro de categorías en el almacén (esperado false): %s" % \
		_panel._grilla_cofre._tabs_filtro.visible)
	print("Sin selector de orden en el almacén (esperado false): %s" % \
		_panel._grilla_cofre._fila_orden.visible)
	var sin_filtro_ni_orden_ok: bool = not _panel._grilla_cofre._tabs_filtro.visible \
		and not _panel._grilla_cofre._fila_orden.visible

	_se_abre_con_titulo_y_contenido_del_almacen_ok = titulo_ok and contenido_ok and sin_filtro_ni_orden_ok


func _probar_bloquea_depositar() -> void:
	var pocion := load("res://recursos/items/consumibles/pocion_vida.tres") as DatosItem
	_inventario.agregar_item(pocion, 1, true)
	_panel._grilla_jugador.notificar_cambio()

	var origen := _casilla_con_nombre(_panel._grilla_jugador, "Poción de Vida")
	var scroll_almacen = _panel._grilla_cofre.get_node("%ScrollContainer")
	scroll_almacen._drop_data(Vector2.ZERO, origen)

	var entro_al_almacen: bool = _casilla_con_nombre(_panel._grilla_cofre, "Poción de Vida") != null
	print("Arrastrar del inventario al almacén no hace nada (esperado false): %s" % entro_al_almacen)

	var se_quedo_en_inventario: bool = _casilla_con_nombre(_panel._grilla_jugador, "Poción de Vida") != null
	print("... la poción se queda en el inventario del jugador (esperado true): %s" % se_quedo_en_inventario)

	_bloquea_depositar_desde_inventario_ok = not entro_al_almacen and se_quedo_en_inventario


func _probar_retirar() -> void:
	var origen := _casilla_con_nombre(_panel._grilla_cofre, "Mineral de Cristal")
	var scroll_inventario = _panel._grilla_jugador.get_node("%ScrollContainer")
	scroll_inventario._drop_data(Vector2.ZERO, origen)

	var total_en_almacen: int = _gm._almacen.get(_RUTA_MINERAL, 0)
	print("Arrastrar del almacén al inventario retira de verdad, server-validado (esperado 0): %d" % \
		total_en_almacen)

	var mineral_en_inventario: bool = false
	for item: DatosItem in _inventario.items:
		if item.name == "Mineral de Cristal":
			mineral_en_inventario = true
			break
	print("... y el mineral llega al inventario del jugador (esperado true): %s" % mineral_en_inventario)

	_retirar_descuenta_del_almacen_y_da_item_ok = total_en_almacen == 0 and mineral_en_inventario


## Simula al minero depositando mientras el panel del almacén sigue
## abierto — GestorMinero.almacen_actualizado debe refrescar la grilla
## sola, sin que el jugador tenga que cerrar y volver a abrir. El almacén
## quedó en 0 tras _probar_retirar(), así que esto reaparece como fila
## nueva.
func _probar_refresco_en_caliente() -> void:
	var mineral := load(_RUTA_MINERAL) as DatosItem
	_gm.depositar_servidor(mineral)

	var entro_solo: bool = _casilla_con_nombre(_panel._grilla_cofre, mineral.name) != null
	print("Un depósito nuevo mientras el panel está abierto se ve solo (esperado true): %s" % entro_solo)
	_refresco_en_caliente_al_depositar_ok = entro_solo


func _probar_tomar_todo() -> void:
	_panel._grilla_cofre.tomar_todo_solicitado.emit()

	print("'Tomar todo' vacía el almacén (esperado true): %s" % _gm._almacen.is_empty())
	var almacen_vacio_ok: bool = _gm._almacen.is_empty()

	var mineral_en_inventario: bool = false
	for item: DatosItem in _inventario.items:
		if item.name == "Mineral de Cristal":
			mineral_en_inventario = true
			break
	print("... y lo que quedaba (mineral) llega al inventario (esperado true): %s" % mineral_en_inventario)

	_tomar_todo_vacia_el_almacen_ok = almacen_vacio_ok and mineral_en_inventario


func _informar() -> bool:
	var exito := _se_abre_con_titulo_y_contenido_del_almacen_ok and _bloquea_depositar_desde_inventario_ok \
		and _retirar_descuenta_del_almacen_y_da_item_ok and _refresco_en_caliente_al_depositar_ok \
		and _tomar_todo_vacia_el_almacen_ok
	print("  se abre con título y contenido del almacén: %s" % _se_abre_con_titulo_y_contenido_del_almacen_ok)
	print("  bloquea depositar desde el inventario: %s" % _bloquea_depositar_desde_inventario_ok)
	print("  retirar descuenta del almacén y da el ítem: %s" % _retirar_descuenta_del_almacen_y_da_item_ok)
	print("  refresco en caliente al depositar: %s" % _refresco_en_caliente_al_depositar_ok)
	print("  'Tomar todo' vacía el almacén: %s" % _tomar_todo_vacia_el_almacen_ok)
	print("PRUEBA PANEL COFRE ALMACEN MINERO %s" % ("OK" if exito else "FALLIDA"))
	quit(0 if exito else 1)
	return true
