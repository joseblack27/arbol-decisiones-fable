# =============================================================================
# Prueba de integración completa del Núcleo de la Forja (jefe de la Forja de
# Fuego, ver el plan "Mina — Forja de Fuego" en C:\Users\USER\.claude\plans\
# cheeky-mixing-melody.md) — mirror de prueba_cristal_4_fases_completas.gd.
#   1. Fase 1 -> 2 (<=0.75): golpe de transición pega, se suman Barrido de
#      Brasas y Muro de Forja al selector.
#   2. Fase 2 -> 3 (<=0.50): golpe de transición pega, se suman Terremoto de
#      Forja y Combo de Brasas.
#   3. Fase 3 -> 4 (<=0.25): golpe de transición pega, se suma Embestida al
#      Rojo Vivo, se invocan los 2 refuerzos (Lobo + Araña), se activa la
#      furia final.
#   4. Tras las 3 transiciones, el jefe sigue siendo matable de verdad.
#   godot --headless --path . --script res://pruebas/prueba_forja_4_fases_completas.gd
# =============================================================================
extends SceneTree
...
func _montar() -> void:
	...
	_jefe = (load("res://escenas/enemigos/EnemigoNucleoForja.tscn") as PackedScene).instantiate()
...
func _verificar_fase2() -> void:
	...
	var tiene_barrido: bool = selector != null and selector.habilidades.has(_jefe.habilidad_barrido_bt)
	var tiene_muro: bool = selector != null and selector.habilidades.has(_jefe.habilidad_muro_bt)
	_fase1_a_2_habilidades_ok = tiene_barrido and tiene_muro
	print("Fase 2 suma Barrido + Muro al selector correcto (esperado true): %s" % _fase1_a_2_habilidades_ok)
...
func _verificar_fase3() -> void:
	...
	var selector = _jefe.get_node_or_null("ArbolComportamiento/Selector/Atacar/SelectorHabilidades")
	var tiene_terremoto: bool = selector != null and selector.habilidades.has(_jefe.habilidad_terremoto_bt)
	var tiene_combo: bool = selector != null and selector.habilidades.has(_jefe.habilidad_combo_bt)
	_fase2_a_3_habilidades_ok = tiene_terremoto and tiene_combo
	print("Fase 3 suma Terremoto + Combo al selector correcto (esperado true): %s" % _fase2_a_3_habilidades_ok)
...
func _informar() -> bool:
	...
	print("PRUEBA FORJA 4 FASES COMPLETAS %s" % ("OK" if exito else "FALLIDA"))
