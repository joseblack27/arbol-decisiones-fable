# =============================================================================
# Prueba de integración completa del Corazón de Cristal (jefe de la Mina de
# Cristal, ver el plan "Mina de Cristal" en C:\Users\USER\.claude\plans\
# cheeky-mixing-melody.md) — mirror de prueba_guardian_4_fases_completas.gd
# + la parte de refuerzos de prueba_guardian_fase4.gd, combinadas en un solo
# archivo: a diferencia del Guardián (charco/escudo/castigo — mecánicas
# únicas por fase que ameritaban su propia prueba dedicada), acá TODO el kit
# son habilidades genéricas ya probadas por otros mobs (Golpe Básico,
# Arañazo, Proyectil, Área de Efecto, Muro, Sacudida, Carga — ver
# EnemigoCorazonCristal.gd para el porqué completo de cada reuso), así que
# lo único que hace falta verificar acá es la MECÁNICA DE FASES en sí:
#   1. Fase 1 -> 2 (<=0.75): golpe de transición pega, se suman Lluvia de
#      Esquirlas y Erupción de Cristal al selector.
#   2. Fase 2 -> 3 (<=0.50): golpe de transición pega, se suman Muro de
#      Cristal y Pulso de Cristal.
#   3. Fase 3 -> 4 (<=0.25): golpe de transición pega, se suma Embestida de
#      Cristal, se invocan los 2 refuerzos (Lobo + Araña) y se activa la
#      furia final (multiplicador_recarga).
#   4. Tras las 3 transiciones, el jefe sigue siendo matable de verdad.
#   godot --headless --path . --script res://pruebas/prueba_cristal_4_fases_completas.gd
# =============================================================================
extends SceneTree
...
func _montar() -> void:
	...
	_jefe = (load("res://escenas/enemigos/EnemigoCorazonCristal.tscn") as PackedScene).instantiate()
	...
	# Defensa alta a propósito: el golpe de transición usa ignora_defensa=true,
	# así que tiene que pegar casi entero IGUAL con esto puesto.
...
func _verificar_fase2() -> void:
	...
	var tiene_esquirlas: bool = selector != null and selector.habilidades.has(_jefe.habilidad_esquirlas_bt)
	var tiene_erupcion: bool = selector != null and selector.habilidades.has(_jefe.habilidad_erupcion_bt)
	_fase1_a_2_habilidades_ok = tiene_esquirlas and tiene_erupcion
	print("Fase 2 suma Esquirlas + Erupción al selector correcto (esperado true): %s" % _fase1_a_2_habilidades_ok)
...
func _verificar_fase3() -> void:
	...
	var tiene_pulso: bool = selector != null and selector.habilidades.has(_jefe.habilidad_pulso_bt)
	_fase2_a_3_habilidades_ok = tiene_muro and tiene_pulso
	print("Fase 3 suma Muro + Pulso al selector correcto (esperado true): %s" % _fase2_a_3_habilidades_ok)
...
func _verificar_fase4() -> void:
	...
	# 2 (fase1) + 2 (fase2) + 2 (fase3) + 1 (fase4).
...
## invulnerabilidad temporal, reanudaciones), el jefe TIENE que seguir
## siendo un enemigo normal y matable.
...
func _informar() -> bool:
	...
	print("PRUEBA CRISTAL 4 FASES COMPLETAS %s" % ("OK" if exito else "FALLIDA"))
