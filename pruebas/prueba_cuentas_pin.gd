# =============================================================================
# Prueba de cuentas por nombre+PIN (GestorCuentas.resolver_cuenta):
...
func _probar() -> bool:
	...
	var gc := root.get_node("/root/GestorCuentas")
	...
	var clave: String = gc.call("_normalizar_nombre_cuenta", NOMBRE_PRUEBA)
	bd.query_with_bindings("DELETE FROM cuentas WHERE nombre = ?;", [clave])

	# 1. Cuenta nueva: adopta el id del dispositivo que la crea.
	var id_creacion: String = gc.call("resolver_cuenta", NOMBRE_PRUEBA, PIN_CORRECTO, ID_DISPOSITIVO_1)
	...
	var id_reconexion: String = gc.call("resolver_cuenta", NOMBRE_PRUEBA, PIN_CORRECTO, ID_DISPOSITIVO_2)
	...
	var id_pin_malo: String = gc.call("resolver_cuenta", NOMBRE_PRUEBA, "0000", ID_DISPOSITIVO_2)
	var rechazo_ok := id_pin_malo == ""
	print("PIN incorrecto rechaza (esperado \"\"): '%s'" % id_pin_malo)

	# 4. Nombre normalizado (mayúsculas/espacios) resuelve a la misma cuenta.
	var id_variante: String = gc.call("resolver_cuenta", "  PruebaCuentaPin  ", PIN_CORRECTO, ID_DISPOSITIVO_2)
