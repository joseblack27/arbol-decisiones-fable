...
## Núcleo de la Forja — jefe de la rama "Forja de Fuego" de la Mina (ver el
## plan "Mina — Forja de Fuego" en C:\Users\USER\.claude\plans\
## cheeky-mixing-melody.md). Mirror EXACTO de EnemigoCorazonCristal.gd (ver
## ese archivo para el porqué completo de cada decisión: reuso de
## habilidades genéricas en vez de escribir código nuevo por jefe, patrón de
## fases calcado de EnemigoGuardianQuebrado.gd) — solo cambian los nombres
## de campos/habilidades por el tema de fuego/forja.
...
func _conectar_embestida() -> void:
	var embestida := get_node_or_null("Habilidades/HabilidadEmbestidaForja")
	if embestida == null:
		return
...
func _reanudar_fase(fase: int) -> void:
	match fase:
		2:
			_agregar_habilidad_bt("ArbolComportamiento/Selector/Atacar/SelectorHabilidades", habilidad_barrido_bt)
			_agregar_habilidad_bt("ArbolComportamiento/Selector/Atacar/SelectorHabilidades", habilidad_muro_bt)
		3:
			_agregar_habilidad_bt("ArbolComportamiento/Selector/Atacar/SelectorHabilidades", habilidad_terremoto_bt)
			_agregar_habilidad_bt("ArbolComportamiento/Selector/Atacar/SelectorHabilidades", habilidad_combo_bt)
		4:
