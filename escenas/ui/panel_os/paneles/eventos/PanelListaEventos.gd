...
signal event_selected(event: DatosEvento)

func build(events: Array[DatosEvento]):
	clear()
	GestorEventos.event_expired.connect(_on_event_expired)

	for event in events:
		var button: EventoOpcionListaBoton = event_button_scene.instantiate()
...
func _on_event_pressed(event: DatosEvento):
	GestorEventos.event_selected = event
	event_selected.emit(event)

func _on_event_expired(expired_event: DatosEvento):
	GestorEventos.delete_event_by_id(expired_event.id)

	if event_buttons.has(expired_event.id):
		if GestorEventos.event_selected == expired_event:
